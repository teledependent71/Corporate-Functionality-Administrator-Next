...
import postsPageInitialPathsE5518Resource from '../../resources/posts-page-initial-paths-e5518'
import postsPageInitialPropsF08f4Resource from '../../resources/posts-page-initial-props-f08f4'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Corporate Functionality Administrator</title>
          <meta
            property="og:title"
            content="Posts1 - Corporate Functionality Administrator"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsE5518Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsF08f4Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
